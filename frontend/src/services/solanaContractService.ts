...
export class SolanaContractService {
  ...
  constructor() {
    ...
    this.connection = new (require("@solana/web3.js").Connection)(
      endpoint,
      "confirmed",
    );
    this.programId = new (require("@solana/web3.js").PublicKey)(
  ...
  async donate(params: SolanaDonationParams): Promise<SolanaDonationResponse> {
    ...
    if (!provider) {
      throw new Error(
        "No Solana wallet found. Please install Phantom or use MetaMask with Solana support.",
      );
    }
    ...
    const donorPubkey = new (require("@solana/web3.js").PublicKey)(
      provider.publicKey.toString(),
    );
    const recipientPubkey = new (require("@solana/web3.js").PublicKey)(
      recipient,
    );

    // Convert SOL to lamports
    const lamports = Math.floor(
      parseFloat(amount) * require("@solana/web3.js").LAMPORTS_PER_SOL,
    );

    // Derive PDAs
    const [programStateAccount] =
      await require("@solana/web3.js").PublicKey.findProgramAddress(
        [Buffer.from("program_state")],
        this.programId,
      );

    const [projectDonationsAccount] =
      await require("@solana/web3.js").PublicKey.findProgramAddress(
        [Buffer.from("project_donations"), recipientPubkey.toBuffer()],
        this.programId,
      );

    const [donorDonationsAccount] =
      await require("@solana/web3.js").PublicKey.findProgramAddress(
    ...
    const programStateInfo =
      await this.connection.getAccountInfo(programStateAccount);
    if (!programStateInfo) {
      throw new Error(
        "Program not initialized. Please contact the administrator.",
      );
    }
  ...
  private deserializeProgramState(data: Buffer): ProgramState {
    ...
    // Read owner (32 bytes)
    const owner = new (require("@solana/web3.js").PublicKey)(
      data.slice(offset, offset + 32),
    );
    offset += 32;

    // Read fee wallet (32 bytes)
    const feeWallet = new (require("@solana/web3.js").PublicKey)(
      data.slice(offset, offset + 32),
    );
    offset += 32;
  ...
  private createDonateInstruction(
    ...
    const data = Buffer.alloc(
      1 + 8 + 4 + cryptoType.length + 4 + message.length + 1,
    );
    let offset = 0;
    ...
    return new (require("@solana/web3.js").TransactionInstruction)({
      keys: [
        ...
        {
          pubkey: require("@solana/web3.js").SystemProgram.programId,
          isSigner: false,
          isWritable: false,
        },
      ],
