...
async function fetchPriceFromCMC(symbol: string, slug: string | null): Promise<number> {
  const queryParam = slug ? `slug=${slug}` : `symbol=${symbol}`;
  const apiKey = process.env.NEXT_CMC_PRO_API_KEY;
  const apiUrl = process.env.NEXT_PUBLIC_CMC_PRO_API_URL || "https://pro-api.coinmarketcap.com";
      ...
      try {
        ...
        return NextResponse.json(
          { 
            ...
            details: {
              coinGecko: coinGeckoError instanceof Error ? coinGeckoError.message : "Unknown error",
              cmc: cmcError instanceof Error ? cmcError.message : "Unknown error"
            }
          },
          { status: 503 },
        );
