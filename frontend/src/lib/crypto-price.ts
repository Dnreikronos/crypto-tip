...
class CryptoPriceService {
  ...
  private isRateLimited(apiKey: string): boolean {
    ...
    if (now > limit.resetTime) {
      ...
      this.rateLimits.set(apiKey, { requests: 0, resetTime: now + this.RATE_LIMIT_WINDOW });
      return false;
    }
  ...
  private incrementRateLimit(apiKey: string): void {
    ...
    if (!limit || now > limit.resetTime) {
      this.rateLimits.set(apiKey, { requests: 1, resetTime: now + this.RATE_LIMIT_WINDOW });
    } else {
      limit.requests++;
    }
  }
  ...
  private async fetchFromCoinMarketCap(symbol: string): Promise<number> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`CoinMarketCap API error: ${response.status} - ${errorData.error || response.statusText}`);
    }
    ...
    if (typeof data.price !== 'number' || data.price <= 0) {
      throw new Error(`Invalid price from CoinMarketCap: ${JSON.stringify(data)}`);
    }
  ...
  async getPrice(symbol: string, forceRefresh = false): Promise<number> {
    ...
    const apis = [
      ...
      { name: 'CoinMarketCap', fetch: () => this.fetchFromCoinMarketCap(symbol) },
    ...
    for (const api of apis) {
      try {
        ...
        console.log(`Successfully fetched ${symbol} price from ${api.name}:`, price);
        return price;
      } catch (error) {
  ...
  // Get cache status
  getCacheInfo(): Array<{ symbol: string; price: number; age: number; valid: boolean }> {
    const now = Date.now();
  ...
  getRateLimitInfo(): Array<{ api: string; requests: number; resetIn: number }> {
    const now = Date.now();
